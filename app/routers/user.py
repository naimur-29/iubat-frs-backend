...
def get_users(db: Session=Depends(get_db), limit: int = 20, skip: int = 0, username: Optional[str]=""):
...
@router.get("/{id}", response_model=schemas.PostUserResponse)
...
@router.put("/cpass/{id}", status_code=200)
